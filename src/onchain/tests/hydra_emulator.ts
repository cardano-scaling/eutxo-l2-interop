...
export class HydraEmulator implements Provider {
  network?: Network;
  private state: EmulatorState;

  private constructor(network: Network, state: EmulatorState) {
  ...
  static withAccounts(
    ...
    const state = new EmulatorState(time, utxos);
    return new HydraEmulator(network, state);
  }

  static withUtxos(utxos: Utxo[]): HydraEmulator {
    const time = Date.now();
    const network = { Emulator: time };
    const state = new EmulatorState(time, utxos);
    return new HydraEmulator(network, state);
