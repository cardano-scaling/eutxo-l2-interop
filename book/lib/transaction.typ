...
#let vanilla_transaction(name, inputs: (), outputs: (), signatures: (), certificates: (), withdraws: (), mint: (:), validRange: none, notes: none) = context {
  ...
  let boxHeight = 100pt + if certificates.len() > 0 { 32pt * certificates.len() } else { 0pt } + if signatures.len() > 0 { 32pt * signatures.len() } else { 0pt } + if withdraws.len() > 0 { 40pt * withdraws.len() } + if valid_range != none {20pt } else { 0pt }
